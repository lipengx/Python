	...
	try:
		...
		for row in cursor:
			...
			if out["PROCESSTYPE"] == 'A' and out["PROCESSFLAG"] == '2':
				quary = "select * from his_accoreq where HISBATCHDATE = '%s'"%(str(dataTime))
				cursor.execute(quary)
				for row in cursor:
					out = {}
					for i,col in enumerate(cursor.description):
...
def update_data_postgre(dict):
  ...
  # 		}
		...
		quary = "UPDATE public.tc_open_transactions SET TRADEACCO = '%s',res_acceptstatus ='%s',custno = '%s',res_acceptcode = '%s',res_acceptmsg = '%s',res_confirmflag = '%s'where hisrequestno = '%s'"%(str(dict["TRADEACCO"]).decode("GBK"),str(dict["ACCEPTSTATUS"]).decode("GBK"),str(dict["CUSTNO"]).decode("utf8"),str(dict["ACCEPTCODE"]).decode("GBK"),str(dict["ACCEPTMSG"]).decode("GBK"),str(dict["CONFIRMFLAG"]).decode("GBK"),str(dict["HISREQUESTNO"]).decode("GBK"))
		print quary
