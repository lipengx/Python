...
def check_add(code):
	quary_oracle = "select * from %s where INNER_CODE = '%s' AND DECLAREDATE = (select MAX(DECLAREDATE) from %s where INNER_CODE = '%s')"%("PGENIUS.TRUST_INCM",str(code),"PGENIUS.TRUST_INCM",str(code))	
	print "=========================== checking ==============================="
	print quary_oracle
	...
	quary_postgre = "select * from %s where 产品id = '%s' AND 时间 = (select MAX(时间) from %s where 产品id = '%s') "%("private_fund.income",str(code),"private_fund.income",str(code))
	...
	print str(dict_postgre["时间"]),type(dict_postgre["时间"])
	print str(dict_oracle["DECLAREDATE"]),type(dict_oracle["DECLAREDATE"])
	a = str(dict_postgre["时间"]).strip()
	a = time.strptime(a,"%Y-%m-%d %H:%M:%S")
	b = time.strptime(str(dict_oracle["DECLAREDATE"]),"%Y-%m-%d %H:%M:%S")
	print "sss*************************compare to time*********************************************"
	if a == b:
		print "没有增量,不存在增量".decode("utf8")
	else:
	   	print "增量添加中".decode("utf8")
		quary = "select * from PGENIUS.TRUST_INCM where INNER_CODE ='%s' and DECLAREDATE > to_date('%s','YYYY-MM-DD HH24:MI:SS') and DECLAREDATE <= to_date('%s','YYYY-MM-DD HH24:MI:SS')"%(str(code),str(dict_postgre["时间"]).strip(),str(dict_oracle["DECLAREDATE"]))
		print "======================"
...
def check_exist(cursor,code):
	...
	print "=======================********************====================================="
	codes = set(codes)
	if string.atoi(str(code[0])) in codes:
		print code[0],"已经存在 ".decode("utf8")	
		check_add(code[0])
	else:	
		print "wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww",code[0],"不存在".decode("utf8")
...
def searchr_postgre():
	...
	cursor = conn.cursor()
	quary = "select 产品id from private_fund.income"
...
def operation_date_update():
	try:
		...
		for fund_code in results_array:
			...
			quary = "select * from %s where INNER_CODE = %s"%("PGENIUS.TRUST_INCM",fund_code[0])
			searcher_oracle(quary,fund_code)
			print "\n"
	except Exception, e:
		print "result_array is wrong ",e				
	writer_csv()
...
def insert_data_postgre(dict):
	...
	try:
		...
		"单位净值":str(dict["UNT_NET"]),
		"单位累计净值":str(dict["UNT_ACCUM_NET"]),
		"时间":str(dict["DECLAREDATE"]),
		"基金收益":"Null",
		"基金收益率":"Null",
		"万分基金单位收益率":"Null",
		"更改时间":str(time_now).decode("utf8")
  		}
		...
		quary = 'insert into private_fund.income(%s) VALUES(%s)'%(key_string,values_string)
		print quary
		cursor.execute(quary)
		conn.commit()
...
def writer_csv():
	...
	quary_searcher = "select * from private_fund.income where 更改时间 >= '%s'"%time_str
