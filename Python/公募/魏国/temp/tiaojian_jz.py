...
def check_add(code):
	...
	quary = "select * from TFUNDMARKET where vc_fundcode ='%s' and vc_navdate > to_date('%s','YYYY-MM-DD HH24:MI:SS') and vc_navdate <= to_date('%s','YYYY-MM-DD HH24:MI:SS')"%(str(code[0]),dict_postgre["时间"],str(dict_oracle["VC_NAVDATE"]))
	# quary = "select * from TFUNDMARKET where vc_fundcode ='%s' and vc_navdate between %s and %s "%(str(code[0]),dict_postgre["时间"],str(dict_oracle["VC_NAVDATE"]))
...
def operation_date_update():
	try:
		...
		for code in results_array:
			print code
			quary = "select  * from %s where VC_FUNDCODE = '%s' and vc_date >= '20150611'"%("TFUNDMARKET",code[0])	
			print quary
