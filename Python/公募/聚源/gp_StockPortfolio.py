...
def check_add(code):
	quary_sql_server = "select * from %s where InnerCode = '%s' AND InfoPublDate = (select MAX(InfoPublDate) from %s where InnerCode = '%s')"%("dbo.MF_KeyStockPortfolio",str(code),"dbo.MF_KeyStockPortfolio",str(code))	
	print "===========================checking================================"
	...
	dict_sql_servere = dict_max_cursor(cursor_oracle)

	print "=====================================oracle->postgres========================"

	quary_postgre = "select * from %s where 基金代码 = '%s' AND 公布日期 = (select MAX(公布日期) from %s where 基金代码 = '%s') "%("public_fund.stock_gp",str(code),"public_fund.stock_gp",str(code))
	...
	quary = "select * from dbo.MF_KeyStockPortfolio where INNER_CODE ='%s' and InfoPublDate > to_date('%s','YYYY-MM-DD HH24:MI:SS') and InfoPublDate <= to_date('%s','YYYY-MM-DD HH24:MI:SS')"%(str(code),str(dict_postgre["公布日期"]),str(dict_oracle["InfoPublDate"]))

	insert_update_data_postgre(quary,code)
	print "增量处理结束".decode("utf8")
	conn_oracle.close()
	conn_postgre.close()
...
def searchr_postgre():
	...
	cursor = conn.cursor()
	quary = "select 基金代码 from public_fund.stock_gp"
	cursor.execute(quary)
...
def operation_date_update():
	try:
		...
		print "*********results_array is leghth ******",len(results_array)
		for code in results_array:	
			...
			if len(result_code) > 0:
				quary = "select  * from dbo.MF_KeyStockPortfolio where InnerCode = '%s'"%(result_code[0][0])	
				searcher_sql_server(quary,result_code[0][0],code[0])
				print "\n"
			else:
				print code[0],"没有数据".decode("utf8")
		print "\n"
		writer_csv()
	except Exception, e:
...
def dict_update_cursor(cursor,code):
	try:
		print "....开始转换。。。".decode("utf8")
		ross = cursor.fetchall()
		if len(ross) > 0:	
			print "data to dictinary ....................changing ..........."	
			index = cursor.description
			result = []
			for res in ross:
				out = {}
...
def insert_data_postgre(dict,code):
	...
	try:
		...
		"配置id":"Null",
		"基金代码":code,
		"股票代码":str(dict["StockInnerCode"]),
		"股票名称":"Null", 
		"持股数量":str(dict["SharesHolding"]),
		"市值":str(dict["MarketValue"]),
		"占资产净值比例":str(dict["RatioInNV"]),
		"公布日期":str(dict["InfoPublDate"]),
		"修改时间":str(time_now)
		}
		# InnerCode_quary = "select ChiName from dbo.LC_StockArchives where CompanyCode =(select CompanyCode from dbo.LC_CodeRelationship where InnerCode = '%s')"%(str(dict["StockInnerCode"]))
		InnerCode_quary = "select * from dbo.SecuMain where InnerCode = '%s'"%(str(dict["StockInnerCode"]))
		conns = connect_sql_server()
		cursors = conns.cursor()
		cursors.execute(InnerCode_quary)
		print "*******************",InnerCode_quary
		result_code = cursors.fetchall()

		for row in cursors:
			out = {}
			for i,col in enumerate(cursors.description):		
				out[col[0]] = row[i]
			param["股票名称"] = out["SecuAbbr"]
			param["股票代码"] = out["SecuCode"]
		...
		quary = "insert into public_fund.stock_gp (%s) values(%s)"%(key_string,values_string)
		print quary	
		cursor.execute(quary)
		conn.commit()
...
def writer_csv():
	...
	quary_searcher = "select * from public_fund.stock_gp where 修改时间 >= '%s'"%time_str
