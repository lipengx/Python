...
def check_add(code):
	quary_sql_server = "select * from %s where InnerCode = '%s' AND InfoPublDate = (select MAX(InfoPublDate) from %s where InnerCode = '%s')"%("dbo.MF_BondPortifolioStru",str(code[0]),"dbo.MF_BondPortifolioStru",str(code[0]))	
	print "===========================checking================================"
	...
	quary_postgre = "select * from %s where 基金代码 = '%s' AND 公布日期 = (select MAX(公布日期) from %s where 基金代码 = '%s') "%("public_fund.stock_zq",str(code[0]),"public_fund.stock_zq",str(code[0]))
	...
	quary = "select * from dbo.MF_BondPortifolioStru where InnerCode ='%s' and InfoPublDate between %s and %s"%(str(code[0]),dict_postgre["公布日期"],str(dict_oracle["InfoPublDate"]))
...
def searchr_postgre():
	...
	cursor = conn.cursor()
	quary = "select 基金代码 from public_fund.stock_zq"
	cursor.execute(quary)
...
def operation_date_update():
	...
	try:
		...
		for code in results_array:
			...
			cursor.close()
			conn.close()
			quary = "select  * from dbo.MF_BondPortifolioStru where InnerCode = '%s'"%(result_code[0][0])	
			print quary
...
def insert_data_postgre(dict,code):
	...
	try:
		...
		"基金代码":code,
		"债券代码":"Null",
		"债券类型":str(dict["BondType"]),
		"市值":str(dict["MarketValue"]),
		"占资产净值比例":str(dict["RatioInNV"]),
		"占债券组合市值比":str(dict["RatioInBondPortfolio"]),
		...
		quary = "insert into public_fund.stock_zq (%s) values(%s)"%(key_string,values_string)		
...
def writer_csv():
	...
	quary_searcher = "select * from public_fund.stock_zq where 修改时间 >= '%s'"%time_str
