...
def check_add(code):
	quary_sql_server = "select * from %s where InnerCode = '%s' AND InfoPublDate = (select MAX(InfoPublDate) from %s where InnerCode = '%s')"%("dbo.MF_AssetAllocation",str(code[0]),"dbo.MF_AssetAllocation",str(code[0]))	
	print "===========================checking================================"
	...
	quary_postgre = "select * from %s where 基金代码 = '%s' AND 公布日期 = (select MAX(公布日期) from %s where 基金代码 = '%s') "%("public_fund.money_set",str(code[0]),"public_fund.money_set",str(code[0]))
	...
	quary = "select * from dbo.MF_AssetAllocation where InnerCode ='%s' and InfoPublDate between %s and %s"%(str(code[0]),dict_postgre["公布日期"],str(dict_oracle["InfoPublDate"]))
...
def searchr_postgre():
	...
	cursor = conn.cursor()
	quary = "select 基金代码 from public_fund.money_set"
	cursor.execute(quary)
	...
	try:
		...
		for code in results_array:
			...
			if len(result_code) > 0:
				...
				quary = "select  * from dbo.MF_AssetAllocation where InnerCode = '%s'"%(result_code[0][0])	
				print quary
...
def insert_data_postgre(dict,code):
	...
	try:
		...
		"资产配置id":"Null",
		"基金代码":code,
		"资产总类代码":str(dict["AssetTypeCode"]),
		"资产总类":str(dict["AssetType"]),
		"资产市值":str(dict["MarketValue"]),
		"资产总值占比":str(dict["RatioInTotalAsset"]),
		"资产净值占比":str(dict["RatioInNV"]),
		...
		quary = "insert into public_fund.money_set (%s) values(%s)"%(key_string,values_string)		
...
def writer_csv():
	...
	quary_searcher = "select * from public_fund.money_set where 修改时间 >= '%s'"%time_str
